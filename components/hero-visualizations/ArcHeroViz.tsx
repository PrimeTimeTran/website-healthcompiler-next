...
const ArcHeroViz = ({
  ...
  return (
    <div className="relative w-full h-[400px] flex items-center justify-center">
      ...
      >
        <defs>
          <linearGradient id={`${id}-coreGradient`} x1="0%" y1="0%" x2="100%" y2="100%">
        ...
        {segments.map((segment, i) => {
          ...
          const endAngle = ((i + 1) * (360 / segments.length) - 90) * (Math.PI / 180)
        ...
        {/* Segment labels */}
        {segments.map((segment, i) => {
          const angle = (i * (360 / segments.length) + 180 / segments.length - 90) * (Math.PI / 180)
        ...
        {/* Data particles */}
        {[0, 1, 2].map((i) => (
          <circle key={`particle-${i}`} r="4" fill="hsl(var(--accent))" opacity="0.8">
            <animateMotion
              dur={`${3 + i * 0.5}s`}
